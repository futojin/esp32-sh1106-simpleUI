...
void HeroPageItem::draw(u_int16_t idx)
{
  ...
  m_display->drawString(m_display->getWidth() / 2, m_offsetY, m_label);

  m_display->setFont(value_font);
  m_display->drawString(m_display->getWidth() / 2, m_offsetY + label_font[1], value);
}
...
void HeroPageItem::drawValueHighlight(u_int16_t idx)
{
  ...
  int16_t y = m_offsetY + label_font[1];
